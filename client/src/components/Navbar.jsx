...
export default function Navbar() {
  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${scrolled ? 'bg-white/80 backdrop-blur-xl border-b border-zinc-200' : 'bg-transparent'}`}>
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 lg:px-8 relative">
        ...
        <a href="#" className="flex items-center gap-3 group">
          ...
          <div className="flex flex-col">
            <span className="text-lg font-bold leading-none tracking-tight text-zinc-900">
              Smart<span className="text-[#4f39f6]">City</span>
            </span>
        ...
        <div className="hidden md:flex items-center gap-5">
          <a href="#login" className="text-sm font-medium text-zinc-500 hover:text-zinc-900 transition-colors">
            Log in
          </a>
          <button className="flex items-center gap-2.5 bg-gradient-to-r from-zinc-950 to-zinc-600 text-zinc-50 hover:text-white text-sm font-medium pl-5 pr-2 py-2 rounded-full cursor-pointer border-0 shadow-md transition-all hover:shadow-lg hover:-translate-y-0.5">
            Report an Issue
      ...
      <div className={`absolute top-full left-0 w-full bg-white border-b border-zinc-200 flex flex-col p-5 gap-1 md:hidden shadow-xl transition-all duration-300 origin-top ${menuOpen ? 'scale-y-100 opacity-100' : 'scale-y-0 opacity-0 pointer-events-none'}`}>
        ...
        <a href="#login" className="px-4 py-2.5 rounded-lg text-sm text-zinc-500 hover:bg-zinc-50 font-medium" onClick={() => setMenuOpen(false)}>
          Log in
        </a>
        <button className="flex items-center justify-center gap-2.5 bg-gradient-to-r from-zinc-950 to-zinc-600 text-zinc-50 text-sm font-medium px-5 py-2.5 rounded-full cursor-pointer border-0 mt-3 w-fit shadow-md">
          Report an Issue
          <span className="size-7 rounded-full bg-white flex items-center justify-center">
            <svg width="12" height="10" viewBox="0 0 12 10" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M.6 4.602h10m-4-4 4 4-4 4" stroke="#3f3f47" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/></svg>
          </span>
