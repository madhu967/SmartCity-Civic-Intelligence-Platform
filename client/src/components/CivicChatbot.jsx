...
export default function CivicChatbot() {
  ...
  return <div className="civic-chatbot">
    ...
    <button type="button" className={`civic-chat-launcher ${isOpen ? 'civic-chat-launcher-open' : ''}`} onClick={() => setIsOpen((current) => !current)} aria-label={isOpen ? 'Close Civic AI' : 'Open Civic AI'}><span className="civic-chat-launcher-pulse" /><span className="civic-chat-launcher-icon">{isOpen ? <X size={22} /> : <MessageCircle size={23} />}</span><span className="civic-chat-launcher-label">Civic AI</span></button>
  </div>;
}
