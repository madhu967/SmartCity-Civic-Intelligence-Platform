...
export default function WorkerCreatePage() {
  ...
  return (
    <main className="dashboard-page min-h-screen bg-white text-slate-900">
      <Navbar isAuthenticated user={admin} onLogout={logout} />
      <aside className={`dashboard-sidebar ${sidebarOpen ? 'dashboard-sidebar-open' : ''}`}>
        <div className="dashboard-sidebar-brand">
          ...
          <button type="button" onClick={() => setSidebarOpen(false)} className="dashboard-close-button">
            <X size={18} />
          </button>
        </div>
      ...
      <section className="dashboard-main">
        <div className="dashboard-mobile-toolbar">
          <button type="button" onClick={() => setSidebarOpen(true)} className="dashboard-mobile-menu-button">
            <Menu size={20} />
          </button>
          <span>Create worker</span>
        </div>
        <div className="dashboard-container">
          <a href="/admin/workers" className="profile-back-link">
            <ArrowLeft size={15} /> Back to workers management
          </a>
          <div className="dashboard-heading-row worker-heading-row">
            <div>
              <p className="dashboard-eyebrow">Staff management</p>
              <h1 className="dashboard-heading">Create a worker</h1>
              <p className="dashboard-description">Create secure credentials and assign a civic service profile.</p>
            </div>
          </div>
          ...
          <form className="worker-form" onSubmit={submit}>
            <div className="worker-form-section">
              ...
              <div className="worker-form-grid">
                <Field label="Full name" name="name" value={form.name} onChange={updateField} placeholder="Alex Morgan" icon={<UserRound size={16} />} />
                <Field label="Email address" name="email" type="email" value={form.email} onChange={updateField} placeholder="alex@smartcity.local" icon={<UserRound size={16} />} />
                <Field label="Phone number" name="phone" value={form.phone} onChange={updateField} placeholder="+1 555 0100" icon={<Phone size={16} />} />
                <Field label="Worker password" name="password" type="password" value={form.password} onChange={updateField} placeholder="Minimum 6 characters" icon={<KeyRound size={16} />} />
              </div>
            </div>
            <div className="worker-form-section">
              <h2>Service assignment</h2>
              <div className="worker-form-grid">
                <SelectField label="Department" name="department" value={form.department} onChange={updateField} options={departments} />
                <SelectField label="Job / skill" name="jobSkill" value={form.jobSkill} onChange={updateField} options={skills} />
                <Field label="Service area / ward" name="serviceArea" value={form.serviceArea} onChange={updateField} placeholder="Ward 04 - North District" />
                <Field label="Location" name="location" value={form.location} onChange={updateField} placeholder="North operations hub" />
                <Field label="Years of experience" name="yearsExperience" type="number" min="0" value={form.yearsExperience} onChange={updateField} placeholder="5" />
                <SelectField label="Availability" name="availability" value={form.availability} onChange={updateField} options={['Available', 'On duty', 'Unavailable']} />
              </div>
            </div>
            <button type="submit" className="dashboard-primary-button worker-submit">Create worker account <span>→</span></button>
          </form>
        </div>
      </section>
    </main>
