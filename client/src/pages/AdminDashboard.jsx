...
function WorkersTable({ workers }) {
  return <section className="dashboard-panel admin-users-panel"><div className="dashboard-panel-heading"><div><h2>Field workers</h2><p>{workers.length} workers created by administrators</p></div><ShieldCheck size={21} /></div><div className="admin-users-list">{workers.length === 0 ? <div className="admin-empty-users">No workers have been created yet.</div> : workers.map((worker) => <div className="admin-user-row" key={worker.id}><div className="dashboard-avatar">{worker.name?.charAt(0).toUpperCase()}</div><div className="admin-user-info"><strong>{worker.name}</strong><span>{worker.email} · {worker.department}</span><small className="admin-worker-meta">{worker.jobSkill} · {worker.serviceArea} · {worker.yearsExperience} years</small></div><div className={`admin-status ${worker.availability === 'Unavailable' ? 'admin-status-inactive' : 'admin-status-active'}`}>{worker.availability}</div></div>)}</div></section>;
}